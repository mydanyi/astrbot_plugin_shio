...
当前主线版本为 **0.5.1**，只声明兼容 AstrBot **4.27.4** 和 QQ **aiocqhttp**。保留此前回复、合并与临时上下文修复；本次补齐官方引用请求的草稿拦截，清理各气泡的首尾空行，并将辅助配图与主模型历史图片标记隔离。这是此前 0.5.5 修复候选的统一编号，不是回退到早期代码；尚未作为稳定版发布。
...
0.5.1 已统一为当前主线版本，尚未作为稳定 Release 发布。安装或测试时：
...
自然参与节奏和 Master 故障状态使用 AstrBot 插件 KV 保存有限的计数与时间；连续消息批次保存在内存。0.5.1 不再把临时群历史写入 conversation，也不会在卸载或重载时扫描、删除已有会话。
