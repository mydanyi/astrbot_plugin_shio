...
def isolate_replyer_contexts(
    ...
    *,
    ...
    referenced_message_ids = {
        str(item.get("reply_to_message_id", "") or "").strip()
        for item in contexts
        if str(item.get("role", "") or "").lower() == "user"
        and str(item.get("sender_id", "") or "").strip() == sender_id
        and str(item.get("reply_to_message_id", "") or "").strip()
    }
    result: list[dict[str, str]] = []
    ...
    for item in contexts:
        ...
        if role == "user":
            ...
            item_sender_id = str(item.get("sender_id", "") or "").strip()
            if not item_sender_id and match:
                item_sender_id = match.group(1).strip()
            message_id = str(item.get("message_id", "") or "").strip()
            keep_assistant_turn = item_sender_id == sender_id
            keep_user = bool(
                keep_assistant_turn
                or (message_id and message_id in referenced_message_ids)
            )
            if keep_user:
                result.append({"role": "user", "content": content})
        elif role == "assistant":
            target_sender_id = str(
                item.get("target_sender_id", "") or ""
            ).strip()
            if target_sender_id:
                keep_assistant = target_sender_id == sender_id
            else:
                keep_assistant = keep_assistant_turn
            if keep_assistant:
