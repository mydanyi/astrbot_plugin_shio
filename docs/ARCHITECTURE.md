...
| 角色化自然表达 | 提供当前人格、上下文和正式模型请求 | 独立开关，仅补充本轮情境与表达指导，不学习群友话术 |
...
### `core/character_dialogue.py`

独立、默认关闭的角色化自然表达模块。通过已有 `on_llm_request` 增补情境理解和表达指导，不修改原有 system prompt 人格内容、历史或工具。正式回复不增加模型调用；参与判断、冷却、合并、分段和 Meme 均沿用原链路。

当本模块和发送前审核都开启，并允许审核参考角色时，复用 AstrBot `PersonaManager.resolve_selected_persona()`，按当前会话的官方规则读取人格，连同本轮适用的主人语气保存为事件局部参考。只传给这一事件的现有审核调用，不保存长期话术、另建人格或复制聊天历史。参考不能覆盖审核规则或身份事实；读取参考失败不终止主回复。人格全文可能增加审核输入长度，用户可单独关闭这项参考。
...
同一真实会话只运行一个当前批次，其余文本按到达顺序等待，引用或媒体保留官方边界：

- 群聊自然交流可收集多人连续文本；不同发送者直接 @ 形成独立请求，按顺序处理；
- 私聊只涉及同一对端；
- 自然批次以最后一个真实事件作为 watermark；直接批次以该提问者最后一次补充为 watermark，其他人闲聊不延长它；
- 只有 watermark 继续一次 `event.request_llm()`；
- 早期文本以批次事实提供，不合成新的 AstrBot 事件；晚于 watermark 的文本不进入旧请求。
